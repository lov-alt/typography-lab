...
interface LoadedFont { name: string; family: string; url: string }
  ...
  const importFont = useCallback((file: File): Promise<LoadedFont> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        if (!styleRef.current) { styleRef.current = document.createElement("style"); document.head.appendChild(styleRef.current); }
        const format = file.name.endsWith(".woff2") ? "woff2" : file.name.endsWith(".woff") ? "woff" : file.name.endsWith(".otf") ? "opentype" : "truetype";
        styleRef.current.textContent += `@font-face{font-family:"${family}";src:url("${url}")format("${format}");font-display:swap;}`;
